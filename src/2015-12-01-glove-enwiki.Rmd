...
library(data.table)
library(ggplot2)
radim_glove_word2vec <- data.table(
  model = c('GloVe', 'word2vec'),
  accuracy = c(0.671, 0.683),
  runtime_minutes = c(252, 518),
  max_ram_mb = c(9400, 628)
)
radim_glove_word2vec = melt.data.table(data = radim_glove_word2vec, id.vars = c('model'), variable.name = 'measure')

ggplot(radim_glove_word2vec, aes(x = model, y = value)) + 
...
```{r, echo = FALSE}
build_vocab <- data.table(
  tool = c('text2vec', 'gensim', 'raw_IO'),
  time_sec = c(1150, 2100, 150)
)
ggplot(build_vocab, aes(x = tool, y = time_sec)) + 
  geom_bar(aes(fill = tool), stat = 'identity') + 
  geom_text(aes(label = time_sec), vjust = 0)
```
...
```{r, eval=T, echo=FALSE, fig.width=5}
my_glove_word2vec <- data.table(
  model = c('text2vec_glove', 'word2vec'),
  accuracy = c(0.759, 0.687))

ggplot(my_glove_word2vec, aes(x = model, y = accuracy)) + 
  geom_bar(aes(fill = model), stat = 'identity') + 
...
resources <- data.table(
  runtime_minutes = c(119, 431, 401),
  max_ram_mb = c(11100, 820, 405),
  action = c('tcm_creation', 'fitting', 'fitting'),
  model = c('GloVe', 'GloVe', 'word2vec')
)

resources = melt.data.table(data = resources, id.vars = c('model', 'action'), variable.name = 'measure')
ggplot(resources, aes(x = model, y = value)) + 
...
```{r, eval=T, echo=FALSE}
resources <- data.table(
  runtime_minutes = c(119, 50, 401),
  max_ram_mb = c(11100, 410, 405),
  action = c('tcm_creation', 'fitting', 'fitting'),
  model = c('GloVe', 'GloVe', 'word2vec')
)
